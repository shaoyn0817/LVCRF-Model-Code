...
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphNetworkCompiler.NodeType;
...
public class MentionHypergraphFeatureManager extends FeatureManager {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		//System.out.println(words[0].form+" "+words[1].form+" "+words[2].form+" "+words[3].form+" ");
		...
		if(NetworkConfig.USE_NEURAL_FEATURES && flag){
			...
			Object input = null;
			if(MentionHypergraphMainBIO_GENIA_BI.neuralType.equals("lstm")) {
				input = new SimpleImmutableEntry<String, Integer>(s, pos);
			}
			else if(MentionHypergraphMainBIO_GENIA_BI.neuralType.equals("mlp")) {
				input = curWord.form;
			}
			else if(MentionHypergraphMainBIO_GENIA_BI.neuralType.equals("continuous")) {
				input = curWord.form;
			}
			int id = lid;
