package org.statnlp.example.mention_hypergraphGENIA;
...
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphInstance.WordsAndTags;
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphInstance;
import org.statnlp.example.mention_hypergraphGENIA.Span;
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphFeatureManager.FeatureType;
...
public class MentionHypergraphMainBIO_GENIA_BI {
	...
	//public static OptimizerFactory optimizer =  OptimizerFactory.getGradientDescentFactoryUsingGradientClipping(BestParamCriteria.BEST_ON_DEV, 0.01, 5);
    ...
    public static HashMap<String, String> cluster = new HashMap<String, String>();	

	//original model is
	//Dev set:   P: 64.40%    R: 6.41%    F: 11.66%
	//Test set:   P: 59.29%    R: 5.48%    F: 10.03%
	public static void main(String[] args) throws Exception{
		GradientDescentOptimizer.decaystep = 30000;  //if do not want to decay, set a very large number.
		boolean serializeModel = false; 
		System.out.println("NACCL model-GENIA");
		boolean readModelIfAvailable = false;  
		NetworkConfig.USE_BATCH_TRAINING = false;
		NetworkConfig.OPTIMIZE_NEURAL = true; 
        NetworkConfig.NUM_THREADS = 30; // 
        NetworkConfig.PRINT_BATCH_OBJECTIVE=true;
        //目前是BI LSTM
    	String dataset = "GENIA";
		int numIterations = 60000;  
		NetworkConfig.REGULARIZE_NEURAL_FEATURES = true;
		NetworkConfig.BATCH_SIZE = 10;  
		NetworkConfig.USE_NEURAL_FEATURES = true;//Boolean.valueOf(args[0]);//!!!!!!!!!!!!!!!!!!!!!1
		neuralType = "bilstm";//args[1];
		String head = "a";//args[2];
		...
		File ff = new File("paths-"+head);
		FileInputStream fin = new FileInputStream(ff);
		byte []reader = new byte[(int)ff.length()];
		fin.read(reader);
		String ss[] = new String(reader).split("\n");
		for(int i = 0; i < ss.length; i++) {
			String c = ss[i].split("	")[0];
			String w = ss[i].split("	")[1];
			cluster.put(w, c); 
		}
		...
		if(NetworkConfig.USE_NEURAL_FEATURES){
			if (neuralType.equals("lstm")) {
				...
				nets.add(new ECRFContinuousFeatureValueProvider(100, Label.LABELS.size()));
		...
		boolean write = false;
		boolean equal = false;
		for(double mentionPenalty = -2.5; mentionPenalty <= 2.5; mentionPenalty += 0.01){
		...
		//write part
				{
						String test_filename = "data/data/"+dataset+"/test.data";
						MentionHypergraphInstance[] testInstances = readData(test_filename, true, false);
						for(MentionHypergraphInstance instance: testInstances){
							maxSize = Math.max(maxSize, instance.size());
						}
						fm.getParam_G().setWeight(mentionPenaltyFeatureIndex, finalmp);
						Instance[] predictions = model.decode(testInstances);
						String outfile = "data/data/"+dataset+"/GENIA-"+dataset+"-F.data"; 
...
//			instance.posTags = posTags.trim().split(" ");
			...
			if(posTags.trim().split(" ").length != words.trim().split(" ").length) {
				br.readLine();
				index++;
				continue;
			}
			...
			for(String span: spans){
				...
				output.add(new Span(intIndices[0], intIndices[1], label));
			}
		...
		br.close();
		System.out.println("pass "+index+" instance.");
