...
public class MentionHypergraphMainBIO_noemb {
	...
	public static void main(String[] args) throws Exception{
        ...
        //目前是BI LSTM
		int numIterations = 10;//Integer.valueOf(args[0]);  
		neuralType = "mlp";//args[1];
        String dataset = "ACE04";//args[2];
		NetworkConfig.USE_NEURAL_FEATURES = true;//Boolean.valueOf(args[3]); 
		//double mh_value = Double.valueOf(args[4]);
		...
		String train_filename = "data/data/"+dataset+"/train2.data"; 
		...
		//write part
		{
			String outfile = "data/data/"+dataset+"/noemb-dev-out.data";
			if(t== 1)
				outfile = "data/data/"+dataset+"/noemb-test-out.data"; 
		...
		for(double mentionPenalty = -2; mentionPenalty <= 2; mentionPenalty += 0.01){
		...
		//write part
		{
			...
			Instance[] predictions = model.decode(testInstances);
			String outfile = "data/data/"+dataset+"/noemb-test-out-fopt.data"; 
			FileOutputStream fout = new FileOutputStream(outfile);
			for(Instance inst: predictions){
				MentionHypergraphInstance instance = (MentionHypergraphInstance)inst;
				List<Span> goldSpans = instance.output;
