package org.statnlp.example.mention_hypergraphBILU;
...
public class MentionHypergraphNetworkCompiler extends NetworkCompiler {
	...
	public BaseNetwork compileLabeled(int networkId, Instance inst, LocalNetworkParam param){
		...
		for(Span span: instance.output){
			...
			for(int pos=span.start; pos<span.end; pos++){
				// deal with U node
				if(pos == span.start && span.start == span.end-1) {
					long curUNode = toNode_U(pos, size, labelId);
					if(!networkBuilder.contains(curUNode)) {
						networkBuilder.addNode(curUNode);
					}
					...
					try{
						networkBuilder.addEdge(tNode, new long[]{curUNode});
				...
				// deal with B node
				if(pos == span.start){
				    long curBNode = toNode_B(pos, size, labelId);
				    if(!networkBuilder.contains(curBNode)){
				    	networkBuilder.addNode(curBNode);
				    }
				    long tNode = toNode_T(pos, size, labelId);
					if(!networkBuilder.contains(tNode)){
						networkBuilder.addNode(tNode);
					...
					try{
						networkBuilder.addEdge(tNode, new long[]{curBNode});
					} catch (NetworkException e){
						// do nothing, edge from T to I already added (two mentions with the same start index)
					}
					prevINode = curBNode;
				...
				// deal with L node
				if(pos == span.end-1){
					long curLNode = toNode_L(pos, size, labelId);
					if(!networkBuilder.contains(curLNode)) {
						networkBuilder.addNode(curLNode);
					}
					try {
						networkBuilder.addEdge(prevINode, new long[]{curLNode});
					} catch (NetworkException e) {
			...
			for(int idx=0; idx<labels.length; idx++){
				...
				if(networkBuilder.contains(iNode)){
					...
					if(childrenList.size() > 1){
						...
						long nextLNode = toNode_L(pos+1, size, label.id);
						networkBuilder.addEdge(iNode, new long[]{nextLNode, nextINode});
					}
				}
				
				
				{//deal with B Node
					long bNode = toNode_B(pos, size, label.id);
					if(networkBuilder.contains(bNode)){
						// Convert two edges (one to X one to next I) to a single hyperedge
						List<long[]> childrenList = networkBuilder.getChildren_tmp(bNode);
						if(childrenList.size() > 1){
							childrenList.clear();
							long nextINode = toNode_I(pos+1, size, label.id);
                            long nextLNode = toNode_L(pos+1, size, label.id);
							networkBuilder.addEdge(bNode, new long[]{nextLNode, nextINode});
						}
					}
				}
				...
				if(!networkBuilder.contains(tNode)){
					...
					List<long[]> childrenList = networkBuilder.getChildren_tmp(tNode);
					if(childrenList.size() > 1) {
						childrenList.clear();
						long nextBNode = toNode_B(pos, size, label.id);
						long nextUNode = toNode_U(pos, size, label.id);
						networkBuilder.addEdge(tNode, new long[]{nextBNode, nextUNode});
					}
				}
				tNodes[idx] = tNode;
			}
		...
		for(int pos=size-1; pos>=0; pos--){
			...
			for(int idx=0; idx<labels.length; idx++){
				...
				if(pos < size-1){
					long nextINode = toNode_I(pos+1, size, label.id);
					long nextLNode = toNode_L(pos+1, size, label.id);
					networkBuilder.addEdge(iNode, new long[]{nextLNode});
					if(pos < size-2) {
				...
				if(pos < size-1) {
				long bNode = toNode_B(pos, size, label.id);
				networkBuilder.addNode(bNode);
				if(pos < size-1){
					long nextINode = toNode_I(pos+1, size, label.id);
					long nextLNode = toNode_L(pos+1, size, label.id);
					networkBuilder.addEdge(bNode, new long[]{nextLNode});
					if(pos < size-2) {
					networkBuilder.addEdge(bNode, new long[]{nextINode});
					networkBuilder.addEdge(bNode, new long[]{nextLNode, nextINode});
					}
				}}
				// U_NODE
				long uNode = toNode_U(pos, size, label.id);
				networkBuilder.addNode(uNode);
				networkBuilder.addEdge(uNode, new long[]{xNode});
				// L_NODE
				if(pos > 0) {
				long lNode = toNode_L(pos, size, label.id);
				networkBuilder.addNode(lNode);
				networkBuilder.addEdge(lNode, new long[]{xNode});
				}
				// T_NODE
				long tNode = toNode_T(pos, size, label.id);
				networkBuilder.addNode(tNode);
				networkBuilder.addEdge(tNode, new long[]{xNode});
				networkBuilder.addEdge(tNode, new long[]{uNode});
				if(pos<size-1) {
			    long bNode = toNode_B(pos, size, label.id);
				networkBuilder.addEdge(tNode, new long[]{bNode});
				networkBuilder.addEdge(tNode, new long[]{bNode, uNode});
				}
	...
	private long toNode_B(int pos, int size, int labelId){
		return toNode(pos, size, NodeType.B_NODE, labelId);
	}
	...
	private long toNode_L(int pos, int size, int labelId){
		return toNode(pos, size, NodeType.L_NODE, labelId);
	}
	
	private long toNode_U(int pos, int size, int labelId){
		return toNode(pos, size, NodeType.U_NODE, labelId);
	}
	...
	public MentionHypergraphInstance decompile (Network net) {
		...
		for(int pos=0; pos<size; pos++){
			...
			for(int idx=0; idx<curTNodes.length; idx++){
				...
				// either an B-node or an X-node
				if(child.length > 1) {
					// U_NODE
					prediction.add(new Span(pos, pos+1, pos, pos+1, label));
					// B_NODE
					int node_k = child[0]; 
					...
					if(nodeType == NodeType.B_NODE){ // Has a mention starting here
						...
						int end = start;
						boolean flag = true;
						while(nodeType == NodeType.L_NODE || nodeType == NodeType.I_NODE || (nodeType == NodeType.B_NODE && flag)){
							flag = false;
							child = network.getMaxPath(node_k);
							if(nodeType == NodeType.B_NODE && child.length != 1){  //
								node_k = child[1];
								node_arr = network.getNodeArray(node_k);
								nodeType = NodeType.values()[node_arr[1]];
								prediction.add(new Span(start, end+2, start, end+2, label));
								end++;
								continue;
							}
							if(child.length == 1){
								node_k = child[0];
							} else { // there are two children for this edge (overlapping mention) c1:L c2:I
								if(processed.contains(child[1])){ 
									unprocessed.remove(node_k);
									node_k = child[0];
								} else {
									unprocessed.put(node_k, new Span(start, end+2, start, end+2, label)); 
									node_k = child[1];
					...
					} else System.err.println("wrong in decoding!");
					continue;
				}
				int node_k = child[0]; 
				int[] node_arr = network.getNodeArray(node_k);
				NodeType nodeType = NodeType.values()[node_arr[1]];
				if(nodeType == NodeType.U_NODE) {
					prediction.add(new Span(pos, pos+1, pos, pos+1, label));
					continue;
				}
				if(nodeType == NodeType.B_NODE){ // Has a mention starting here
