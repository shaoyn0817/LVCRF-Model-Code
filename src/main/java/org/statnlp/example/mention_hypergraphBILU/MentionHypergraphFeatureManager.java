...
import org.statnlp.example.mention_hypergraphBILU.MentionHypergraphNetworkCompiler.NodeType;
...
public class MentionHypergraphFeatureManager extends FeatureManager {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if(children_k.length ==0 )
			System.err.println("wrong compile!");
		...
		if(NetworkConfig.USE_NEURAL_FEATURES){
			Object input = null;
			if(MentionHypergraphMainBIO_BILU.neuralType.equals("lstm")) {
				input = new SimpleImmutableEntry<String, Integer>(s, pos);
			}
			else if(MentionHypergraphMainBIO_BILU.neuralType.equals("mlp")) {
				input = curWord.form;
			}
			else if(MentionHypergraphMainBIO_BILU.neuralType.equals("continuous")) {
				input = curWord.form;
			}
			int id = lid;
