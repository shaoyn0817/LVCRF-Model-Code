...
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphMainBIO_GENIA_BI;
import org.statnlp.example.mention_hypergraphGENIA.MentionHypergraphFeatureManager.FeatureType;
import org.statnlp.example.mention_hypergraphNoEmbGENIA.MentionHypergraphNetworkCompiler.NodeType;
...
public class MentionHypergraphFeatureManager extends FeatureManager {
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k, int children_k_index) {
		...
		if(NetworkConfig.USE_NEURAL_FEATURES && flag){
			...
			Object input = null;
			if(MentionHypergraphMainBIO_noemb_genia.neuralType.equals("lstm")) {
				input = new SimpleImmutableEntry<String, Integer>(s, pos);
			}
			else if(MentionHypergraphMainBIO_noemb_genia.neuralType.equals("mlp")) {
				input = curWord.form;
			}
			else if(MentionHypergraphMainBIO_noemb_genia.neuralType.equals("continuous")) {
				input = curWord.form;
			}
			int id = lid;
	   ...
	   if(moreFeatures){
		for(int child_k: children_k){
			...
			//brown cluster
			String w = curWord.form;
			if(MentionHypergraphMainBIO_GENIA_BI.cluster.containsKey(w)) {
				String c = MentionHypergraphMainBIO_GENIA_BI.cluster.get(w);
				features.add(param_g.toFeature(network, indicator+FeatureType.BROWN+":", labelId, c));
			} else {
				features.add(param_g.toFeature(network, indicator+FeatureType.BROWN+":", labelId, "no"));
			}
