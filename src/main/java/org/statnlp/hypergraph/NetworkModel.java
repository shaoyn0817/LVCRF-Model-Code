...
public abstract class NetworkModel implements Serializable{
	...
	public static int global_step = 0;
	...
	private void evaluateDevelopment(Instance[] devInsts, Function<Instance[], Metric> evalFunction) throws InterruptedException {
		...
		/*
        int validbatch = 30;
		Instance[] devRes = new Instance[devInsts.length];
		int number = devInsts.length/validbatch+1;
		for(int i = 0; i < number; i++) {
			System.out.println("aa:"+i);
		int start = i*validbatch;
		int end = (i+1)*validbatch;
		if(end > devInsts.length)
			end = devInsts.length;
		Instance[] batch = new Instance[validbatch];
		for(int j = start; j < end; j++) {
			batch[j-start] = devInsts[j];
		}
		Instance temp[] = this.decode(batch);
		for(int j = start; j < end; j++) {
			devRes[j] = temp[j-start];
		}
		}
		*/
