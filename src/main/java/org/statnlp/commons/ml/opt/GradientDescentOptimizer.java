...
//		System.err.println("L2norm: "+sum);
		...
		for(int k = 0; k<this._x.length; k++){
			if(currentAdaptiveMethod == AdaptiveMethod.NONE){ // Normal (S)GD
				if(decaystep <= 0) {
					System.err.println("forget to set decay step!!!!!");
				}
				if(decayflag == 0 && NetworkModel.global_step >= decaystep)
				    {
					decayflag = 1;
					System.err.println("learning rate decay to 0.001 at step "+NetworkModel.global_step+".");
